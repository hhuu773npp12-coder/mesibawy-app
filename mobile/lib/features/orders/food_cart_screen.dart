...
class _FoodCartScreenState extends State<FoodCartScreen> {
  ...
  String? get _restaurantKey {
    ...
    for (final it in cart.items) {
      final m = it.offer;
      final rId =
          m['restaurantId']?.toString() ?? (m['restaurant']?['id']?.toString());
  ...
  int get _deliveryFee =>
      _distanceKm == null ? 0 : computeDeliveryFeeIqD(_distanceKm!);
  ...
  Future<void> _computeDistance() async {
    ...
    try {
      final url =
          'https://router.project-osrm.org/route/v1/driving/${origin.longitude},${origin.latitude};${_dest!.longitude},${_dest!.latitude}?overview=false&alternatives=false&steps=false';
  ...
  Future<void> _checkout() async {
    ...
    if (_dest == null) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('اختر موقع التوصيل')));
      return;
    }
    if (_restaurantKey == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'السلة تحتوي عروضاً من مطاعم مختلفة. اطلب من مطعم واحد.',
          ),
        ),
      );
      return;
    ...
    try {
      ...
      for (final it in cart.items) {
        ...
        if (firstOrderId == null && data is Map<String, dynamic>) {
          firstOrderId =
              data['id']?.toString() ?? (data['order']?['id']?.toString());
        }
      }
      cart.clear();
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('تم إرسال الطلب إلى الإدارة')),
      );
      if (firstOrderId != null) {
        Navigator.of(context).pushReplacement(
          MaterialPageRoute(
            builder: (_) => FoodOrderTrackingScreen(orderId: firstOrderId!),
          ),
        );
      } else {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('تعذر إكمال الطلب: ${e.toString()}')),
      );
    } finally {
                ...
                child: ListView.separated(
                  itemBuilder: (_, i) {
                    ...
                    return ListTile(
                      title: Text(name),
                      subtitle: Text(
                        'السعر: $price د.ع × ${it.quantity} = ${price * it.quantity} د.ع',
                      ),
                      trailing: Row(
                  ...
                  children: [
                    if (needsOriginPick) ...[
                      OutlinedButton.icon(
                        ...
                        label: Text(
                          _origin == null
                              ? 'تحديد موقع المطعم (إن لزم)'
                              : 'تم اختيار موقع المطعم',
                        ),
                      ),
                    ...
                    FilledButton(
                      ...
                      child: _busy
                          ...
                          : const Text('اتمام الطلب (الدفع عند الاستلام)'),
                    ),
                  ],
                ),
              ),
